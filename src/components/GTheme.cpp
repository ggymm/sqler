...
QString GTheme::buildGlobalStyleSheet() const {
    ...
    const QString base = QString(
        ...
        "GLabel[g-role=\"title\"] { font-size: 20px; font-weight: 700; color: %2; }"
        "GLabel[g-role=\"subtitle\"] { font-size: 14px; font-weight: 500; color: %4; }"
        "GLabel[g-role=\"caption\"] { font-size: 12px; color: %4; }"
        "GLabel[g-role=\"emphasis\"] { font-size: 14px; font-weight: 600; color: %2; }"
        // Buttons
        "GPushButton { border-radius: %5px; }"
        "GPushButton[g-variant=\"primary\"] { background-color: %6; color: white; border: none; padding: %7px %8px; min-width: %9px; }"
        "GPushButton[g-variant=\"primary\"]:hover { background-color: %10; }"
        "GPushButton[g-variant=\"secondary\"] { background-color: transparent; color: %2; border: 1px solid %11; padding: %7px %8px; min-width: %9px; }"
        "GPushButton[g-variant=\"secondary\"]:hover { background-color: %3; }"
        "GPushButton[g-variant=\"dialog\"] { background-color: %3; border: 1px solid %11; text-align: left; padding: 0px; margin-bottom: %12px; min-height: %13px; }"
        "GPushButton[g-variant=\"dialog\"]:hover { background-color: %14; border-color: %15; }"
        "GPushButton[g-variant=\"toolbar\"] { color: %2; background-color: transparent; border: none; padding: 4px 8px; }"
        "GPushButton[g-variant=\"toolbar\"]:hover { background-color: %11; }"
        // Inputs
        ...
        "GConnectionItem[g-selected=\"true\"] { background-color: %18; }"
        "QLabel[g-status=\"connected\"] { color: %19; }"
        "QLabel[g-status=\"disconnected\"] { color: %4; }"
        // Separator
